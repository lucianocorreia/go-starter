...
func (h *Handler) RenderPage(ctx echo.Context, page Page) error {
	...
	if page.AppName == "" {
		page.AppName = h.Container.Config.App.Name
	}
	...
	buf, err = h.Container.TemplateRenderer.
		Parse().
		Group("page").
		Key(page.Name).
		Base(page.Layout).
	...
	if err != nil {
		return h.Fail(err, "failed to parse and execute templates")
	}
...
func (h *Handler) cachePage(ctx echo.Context, page Page, html *bytes.Buffer) {
	// TODO: implement this
...
func (h *Handler) Redirect(ctx echo.Context, route string, routeParams ...interface{}) error {
...
func (h *Handler) Fail(err error, log string) error {
